...
До первого запуска создайте у DNS-провайдера запись `A`: имя `docs`, значение — публичный IPv4 сервера Timeweb. Убедитесь, что `docs.youtradecs.xyz` уже указывает на сервер.

Установщик собирает `user-docs`, публикует его в `/var/www/youtradecs-docs` и при первом запуске получает отдельный сертификат Let's Encrypt через `certbot`. На сервере должны быть доступны `certbot`, порт 80 и порт 443.
