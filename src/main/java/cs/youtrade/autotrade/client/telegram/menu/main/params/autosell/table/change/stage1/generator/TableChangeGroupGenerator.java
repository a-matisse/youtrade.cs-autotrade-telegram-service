...
public class TableChangeGroupGenerator extends AbstractChangeGenerator<FcdSellChangeGetDto> {
    ...
    public File createFile(FcdSellChangeGetDto input) throws IOException {
        ...
        // Имя таблицы
        String tableName = input.getTokenName();
        ...
        var groups = input.getChangeList(); // returns List<PriceIntervalUpdateV2Dto>
        for (int r = 0; r < groups.size(); r++) {
            PriceIntervalUpdateV2Dto dto = groups.get(r);
        ...
        File tmp = new File(System.getProperty("java.io.tmpdir"), "price_groups.xlsx");
