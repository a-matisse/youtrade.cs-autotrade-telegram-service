package cs.youtrade.autotrade.client.telegram.menu;

import lombok.*;
import org.telegram.telegrambots.meta.api.objects.commands.BotCommand;

import java.util.*;
import java.util.function.Function;
import java.util.stream.Collectors;

@Getter
@ToString
@NoArgsConstructor
@AllArgsConstructor
public enum UserMenu {
    // reserved (menuId = 0)
    START(0, "/start", "Главное меню", false),
    GET_PRICE,
    TOP_UP_STAGE_1,
    TOP_UP_STAGE_P,

    // main menu
    USER(1, "/user", "Меню пользователя", false),
    USER_QUICK_CONFIG_INIT_STAGE_1,
    USER_QUICK_CONFIG_INIT_STAGE_2,
    USER_QUICK_CONFIG_INIT_STAGE_3,
    USER_QUICK_CONFIG_INIT_STAGE_P,
    USER_QUICK_CONFIG_DISABLE,

    // PORTFOLIO_V1
    PORTFOLIO(3, "/portfolio", "Портфолио и история пользователя", false),
    PORTFOLIO_WAITING,
    PORTFOLIO_HISTORY_STAGE_CHOOSE,
    PORTFOLIO_HISTORY_STAGE_1,
    PORTFOLIO_HISTORY_STAGE_P_BUY,
    PORTFOLIO_HISTORY_STAGE_P_SELL,
    // PORTFOLIO_V2
    PORTFOLIO_V2_SELLING_STAGE_1,
    PORTFOLIO_V2_SELLING_STAGE_P,
    PORTFOLIO_V2_INVENTORY_STAGE_1,
    PORTFOLIO_V2_INVENTORY_STAGE_P,
    PORTFOLIO_V2_RESTORE_STAGE_1,
    PORTFOLIO_V2_RESTORE_STAGE_P,

    ACCOUNTS(4, "/accounts", "Управление аккаунтами", false),
    ACCOUNTS_GET,
    ACCOUNTS_ADD_STAGE_CHOOSE,
    // stages for different account adding
    ACCOUNTS_ADD_STAGE_1_BUYER,
    ACCOUNTS_ADD_STAGE_1_SELLER,
    ACCOUNTS_ADD_STAGE_1_WORKER,
    ACCOUNTS_ADD_STAGE_2_BUYER,
    ACCOUNTS_ADD_STAGE_2_WORKER,
    ACCOUNTS_ADD_STAGE_3_WORKER,
    ACCOUNTS_ADD_STAGE_P_BUYER,
    ACCOUNTS_ADD_STAGE_P_SELLER,
    ACCOUNTS_ADD_STAGE_P_WORKER,
    ACCOUNTS_RENAME_STAGE_1,
    ACCOUNTS_RENAME_STAGE_2,
    ACCOUNTS_RENAME_STAGE_P,
    ACCOUNTS_REMOVE_STAGE_CHOOSE,
    ACCOUNTS_REMOVE_STAGE_1,
    ACCOUNTS_REMOVE_STAGE_P,
    ACCOUNTS_TRANSFER_STAGE_1,
    ACCOUNTS_TRANSFER_STAGE_2,
    ACCOUNTS_TRANSFER_STAGE_P,

    // deep params menu
    PARAMS(2, "/deep", "Углублённые настройки", false),
    PARAMS_RENAME_STAGE_1,
    PARAMS_RENAME_STAGE_2,
    PARAMS_RENAME_STAGE_P,
    PARAMS_LIST,
    PARAMS_SWITCH_STAGE_1,
    PARAMS_SWITCH_STAGE_P,
    PARAMS_CREATE_STAGE_1,
    PARAMS_CREATE_STAGE_2,
    PARAMS_CREATE_STAGE_P,
    PARAMS_DELETE_STAGE_1,
    PARAMS_DELETE_STAGE_P,

    AUTOBUY(5, "/autobuy", "Настройки автопокупки", true),
    AUTOBUY_UPDATE_FIELD_STAGE_1,
    AUTOBUY_UPDATE_FIELD_STAGE_2,
    AUTOBUY_UPDATE_FIELD_STAGE_P,
    AUTOBUY_SWITCH_FUNCTION_TYPE,
    AUTOBUY_SWITCH_DUPLICATE_MODE,
    AUTOBUY_GET_NEWEST_ITEMS_STAGE_1,
    AUTOBUY_GET_NEWEST_ITEMS_STAGE_P,

    AUTOSELL(6, "/autosell", "Настройки автопродажи", true),
    AUTOSELL_UPDATE_FIELD_STAGE_1,
    AUTOSELL_UPDATE_FIELD_STAGE_2,
    AUTOSELL_UPDATE_FIELD_STAGE_P,
    AUTOSELL_SWITCH_EVAL_MODE,
    AUTOSELL_SWITCH_EVAL_MODE_S1,

    SCORING(7, "/scoring", "Настройки скоринга", true),
    SCORING_ADD_STAGE_1,
    SCORING_ADD_STAGE_2,
    SCORING_ADD_STAGE_P,
    SCORING_EDIT_STAGE_1,
    SCORING_EDIT_STAGE_2,
    SCORING_EDIT_STAGE_3,
    SCORING_EDIT_STAGE_P,
    SCORING_REMOVE_STAGE_1,
    SCORING_REMOVE_STAGE_P,

    WORDS(8, "/words", "Фильтры слов", true),
    WORDS_ADD_STAGE_CHOOSE,
    WORDS_ADD_STAGE_1,
    WORDS_ADD_STAGE_P,
    WORDS_GET_STAGE_CHOOSE,
    WORDS_GET_STAGE_P,
    WORDS_REMOVE_STAGE_CHOOSE,
    WORDS_REMOVE_STAGE_1,
    WORDS_REMOVE_STAGE_P,
    WORDS_REMOVE_ALL_STAGE_CHOOSE,
    WORDS_REMOVE_ALL_STAGE_P,

    FOLLOW,
    FOLLOW_CHECK,
    FOLLOW_CHECK_ACCEPT,
    FOLLOW_CHECK_DENY,
    FOLLOW_STAGE_CHOOSE,
    FOLLOW_STAGE_1,
    FOLLOW_STAGE_2,
    FOLLOW_STAGE_P,
    FOLLOW_UNFOLLOW_STAGE_1,
    FOLLOW_UNFOLLOW_STAGE_P,

    // preferences menu
    PREFERENCES(9, "/prefs", "Настройки удобства", false),

    REF(10, "/referral", "Меню рефералов", false),
    REF_CREATE,
    REF_CONNECT_STAGE_1,
    REF_CONNECT_STAGE_P,
    REF_TRANSFER_AMOUNT,
    REF_TRANSFER_CONFIRM,
    REF_TRANSFER_PROCEED,
    REF_PAYOUT,

    NOTIFICATION,
    NOTIFICATION_BALANCE;

    private int cmdId;
    private String textCmd;
    private String cmdDescription;
    private boolean qualified;

    private static final Map<String, UserMenu> menuMap;
    private static final List<UserMenu> cmdList;
    private static final Set<UserMenu> notificationSet;

    static {
        menuMap = Arrays
                .stream(UserMenu.values())
                .filter(menu -> menu.textCmd != null)
                .collect(Collectors.toMap(UserMenu::getTextCmd, Function.identity()));

        cmdList = Arrays
                .stream(UserMenu.values())
                .filter(menu -> menu.textCmd != null)
                .sorted(Comparator.comparingInt(UserMenu::getCmdId))
                .toList();

        notificationSet = Arrays
                .stream(UserMenu.values())
                .filter(menu -> menu.name().startsWith("NOTIFICATION"))
                .collect(Collectors.toSet());
    }

    public boolean isNotification() {
        return notificationSet.contains(this);
    }

    public static UserMenu getByTextCmd(String cmd, boolean uQualified) {
        UserMenu menu = menuMap.get(cmd);
        if (menu != null && !(!menu.qualified || uQualified))
            menu = null;
        return menu;
    }

    public static List<BotCommand> getCommands(boolean uQualified) {
        return cmdList
                .stream()
                .filter(menu -> !menu.qualified || uQualified)
                .map(menu -> new BotCommand(menu.getTextCmd(), menu.cmdDescription))
                .toList();
    }
}
